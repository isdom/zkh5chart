/*******************************************************************************
 * File: Ring.js Copyright (C) 2009-2011 Author: L.D.Nigro Contributor(s):
 * 
...
h5chart.Ring = zk.$extends(zk.Widget, {
	...
	setAnimateType : function(value) {
		this._animateType = value;
	},
	...
	setPlot : function(value) {
		...
		l.set( {
			...
			style : '3D',
			pizza : piz,
			pizzaStyle : this._pizzaStyle,
			animateType : this.getAnimateType(),
			type : this._type,
			showValues : shV,
			palette : pal,
			animate : ani,
		...
		this._backImg.getImageData(parentId, Math.max(0,left - 10), Math.max(0,top - 10), w + 12,
				h + 15);
